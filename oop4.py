...
class App(Tk):
    ...
    def __init__(self):
        ...
        self.label_packing = {'fill': 'x', 'pady': (40, 20)}
    ...
    def setup(self, video_downloader: VideoDownloader, progress_recorder: ProgressRecorder, vmr: VideoMusicRemover, cmr: ClipsMusicRemover):
        '''
    ...
    def show_watching_options(self):
        ...
        self.dimensions_cb = Combobox(
            self,
            state='readonly',
            justify='center',
            textvariable=self.clip_dimensions
        )
        self.duration_cb = Combobox(
            self,
            state='readonly',
            justify='center',
        ...
        self.dimensions_cb.current(0)
        self.duration_cb.current(0)

        self.dimensions_cb.pack()
        self.duration_cb.pack(pady=20)
    ...
    def gui_preparation(self, step_name: str = '', clips: list[str] = [], current_clip_index: int = 0):
        ...
        if step_name == 'select clip options':
            self.dimensions_cb.forget()
            self.duration_cb.forget()
